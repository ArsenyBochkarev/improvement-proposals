...
#### `break`-free методы вместо breakable методов
Плюсы:
- Полностью сохраняет совместимость. С этим старый код не обязательно будет переписывать, чтобы перекомпилировать
- Гарантированно можно будет обеспечить ускорение при использовании таких методов (предположение о невозможности встретить `break`)

Минусы:
- Это кажется менее интуитивным при использовании
- Реализация старых методов остаётся прежней, и не будет оптимизирована в предположении о невозможности встретить `break`
